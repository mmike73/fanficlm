"""
db_explorer.py — browser-based explorer for the fanfic knowledge base.

Reads the same SQLite + ChromaDB that the MCP scraper writes to.
Run from the project root:

    python db_explorer.py          # starts on http://localhost:7860
    python db_explorer.py --port 8000

Routes
------
GET  /                  → single-page HTML app
GET  /api/stats         → entity/chunk counts, fandom breakdown
GET  /api/fandoms       → sorted list of unique fandoms
GET  /api/entities      → entity list  (?fandom=  &type=  &q=  for SQL text search)
GET  /api/entities/{id} → entity detail + attributes
GET  /api/search        → semantic search (?q=  &fandom=  &type=  &n=10)
"""
...
# db_explorer.py lives at the project root; mcp_scraper is a sibling package.
...
def api_fandoms():
    entities = database.list_entities()
    fandoms = sorted({e.fandom for e in entities})
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .close-btn { margin-left: auto; background: var(--border); border: none; color: var(--text);
               width: 28px; height: 28px; border-radius: 6px; cursor: pointer; font-size: 1rem; display: flex; align-items: center; justify-content: center; }
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-section">
      ...
      <div class="type-filters" id="type-filters">
        <button class="type-btn active" data-type="" onclick="setType(this)">All</button>
        <button class="type-btn" data-type="character" onclick="setType(this)">Character</button>
        <button class="type-btn" data-type="place" onclick="setType(this)">Place</button>
        <button class="type-btn" data-type="concept" onclick="setType(this)">Concept</button>
        <button class="type-btn" data-type="event" onclick="setType(this)">Event</button>
      </div>
    </div>
    <div class="sidebar-section">
...
let panelData    = {};   // current entity detail (attrs + chunks)
...
// Every API call gets a fresh timestamp — zero chance of browser caching
...
async function doReload() {
  // Sequential: each step independent, errors shown in-place
...
function showSectionError(elId, err) {
  ...
  if (el) el.innerHTML =
    '<span style="color:var(--danger);font-size:.78rem">Error: ' + esc(err.message) + '</span>';
}
...
async function loadEntities() {
  ...
  if (!d.entities || !d.entities.length) {
    grid.innerHTML = '<div class="empty-state" style="grid-column:1/-1">' +
      '<div class="empty-icon">📭</div>No entities stored yet. Run: python scrape.py -i</div>';
    return;
  }
  ...
  d.entities.forEach(e => {
    ...
    card.innerHTML =
      ...
      '</div>' +
      '<div class="card-fandom">' + esc(e.fandom) + ' · ' + esc(e.source_type||'unknown') + '</div>' +
...
async function openEntity(id) {
  ...
  document.getElementById('panel-meta').innerHTML =
    ...
    (e.cache_fresh ? '✓ Fresh' : '⚠ Stale') + ' · ' + (chunks.count||0) + ' chunks</span>';
  ...
  document.getElementById('search-results').innerHTML = d.results.map(r => {
    ...
    return '<div class="result-card">' +
      '<div class="result-header">' +
        ...
        '<span class="badge ' + cls + '" style="margin-left:auto">' + pct + '% · ' + esc(r.chunk_type||'') + '</span>' +
      '</div>' +
